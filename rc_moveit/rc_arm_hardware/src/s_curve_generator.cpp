...
double SCurveGenerator::update(double dt)
{
  ...
  // Step 8：在加速度限制下更新速度
  double prev_velocity = state_.velocity;
  double new_velocity = prev_velocity + state_.acceleration * dt;
  new_velocity = clamp(new_velocity, -max_velocity_, max_velocity_);
  state_.velocity = new_velocity;

  // Step 9：更新位置
  // 使用真正的梯形积分，避免退化成“用新速度积分”导致的过冲
  double avg_velocity = (prev_velocity + new_velocity) * 0.5;
  state_.position += avg_velocity * dt;

  // Step 10：目标附近进入停稳死区，避免反复微振
  const double snap_pos_threshold = std::max(POSITION_THRESHOLD * 10.0, 1e-4);
  const double snap_vel_threshold = std::max(VELOCITY_THRESHOLD * 10.0, 1e-3);
  const double snap_acc_threshold = std::max(max_acceleration_ * 0.05, 1e-2);

  if (std::abs(state_.target_position - state_.position) < snap_pos_threshold &&
      std::abs(state_.velocity) < snap_vel_threshold &&
      std::abs(state_.acceleration) < snap_acc_threshold) {
    state_.position = state_.target_position;
    state_.velocity = 0.0;
    state_.acceleration = 0.0;
    state_.jerk = 0.0;
