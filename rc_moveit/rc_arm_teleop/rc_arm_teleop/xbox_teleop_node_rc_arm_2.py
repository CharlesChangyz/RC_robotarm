...
class XboxTeleopNode(Node):
    def __init__(self):
        super().__init__('xbox_teleop_node_rc_arm_2')
        ...
        self.zero_torque_client = self.create_client(
            SetBool, '/rc_arm/set_zero_torque_mode')
        ...
        self.slave_joint_names = ['j1_joint', 'j2_joint', 'j3_joint', 'j4_joint']
        self.master_joint_names_ms = ['j1_joint', 'j2_joint', 'j3_joint', 'j4_joint']
        ...
        self.get_logger().info('右摇杆Y + LB/RB：Pitch（单欧拉角）')
    ...
    def startup_go_zero(self):
        ...
        for retry in range(max_retries):
            ...
            joint_values = [0.0] * len(self.joint_names)  # All joints to zero
            ...
            if goal_handle and goal_handle.accepted:
                ...
                try:
                    if result and result.result and result.result.error_code.val == result.result.error_code.SUCCESS:
                        ...
                        zero_joint_positions = [0.0] * len(self.joint_names)
    ...
    def go_zero(self):
        ...
        joint_names = list(self.joint_names)
        joint_values = [0.0] * len(self.joint_names)  # Radians
    ...
    def _move_both_to_zero(self):
        ...
        if self.can1_socket is not None:
            ...
            if slave_positions is None:
                slave_positions = [0.0] * len(self.slave_joint_names)
    ...
    def update_callback(self):
        ...
        # Rotation velocities (4-axis arm):
        # only keep one Euler angle (pitch), ignore roll/yaw to fit IK DOF.
        raw_vyaw = 0.0
        raw_vroll = 0.0
        raw_vpitch = (
            self.apply_deadzone(joy.axes[4]) + (joy.buttons[5] - joy.buttons[4])
        ) * current_max_angular  # 右摇杆Y + RB/LB -> Pitch
        ...
        has_rotation = abs(dpitch) > rotation_threshold
        ...
        # Update only pitch for 4-axis IK, keep roll/yaw unchanged
    ...
    def ik_callback(self, future):
        ...
        try:
            ...
            point.velocities = [0.0] * len(self.joint_names)  # Target velocity is 0
