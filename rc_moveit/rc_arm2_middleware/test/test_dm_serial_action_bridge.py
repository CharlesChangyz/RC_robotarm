...
def test_decodes_standard_command_frame_to_action_set_id():
    bridge = DmSerialActionBridge(action_set_ids={1, 2}, command_base_id=0x400, complete_id=0x500)

    action_id = bridge.action_set_id_from_frame(RawCanFrame(can_id=0x401, dlc=0, data=[]))
...
def test_ignores_extended_and_unknown_command_frames():
    ...
    assert bridge.action_set_id_from_frame(
        RawCanFrame(can_id=0x401, dlc=0, data=[], is_extended=True)
    ) is None
    assert bridge.action_set_id_from_frame(RawCanFrame(can_id=0x405, dlc=0, data=[])) is None
...
def test_allows_remote_flag_because_dm_usb_reports_data_frames_as_remote():
    ...
    assert bridge.action_set_id_from_frame(
        RawCanFrame(can_id=0x402, dlc=0, data=[], is_remote=True)
    ) == 2


def test_builds_completion_frame():
    bridge = DmSerialActionBridge(action_set_ids={1}, command_base_id=0x400, complete_id=0x500)
...
def test_empty_allowed_action_set_ids_allows_all_action_sets():
    ...
    bridge = DmSerialActionBridge(action_set_ids=allowed, command_base_id=0x400, complete_id=0x500)

    assert bridge.action_set_id_from_frame(RawCanFrame(can_id=0x405, dlc=0, data=[])) == 5
...
def test_configured_allowed_ids_prevent_other_bus_ids_from_triggering():
    ...
    assert bridge.action_set_id_from_frame(RawCanFrame(can_id=0x402, dlc=0, data=[])) == 2
    assert bridge.action_set_id_from_frame(RawCanFrame(can_id=0x405, dlc=0, data=[])) is None
