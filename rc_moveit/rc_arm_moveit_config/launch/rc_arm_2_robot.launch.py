...
def generate_launch_description():
    ...
    declared_arguments = [
        ...
        DeclareLaunchArgument('middleware_motion_path_topic', default_value='/arm2/middleware/motion_path', description='middleware 发送平滑非直线路径的话题'),
        DeclareLaunchArgument('middleware_cartesian_motion_target_topic', default_value='/arm2/middleware/cartesian_motion_target', description='middleware 发送直线笛卡尔目标点的话题'),
        DeclareLaunchArgument('middleware_cartesian_motion_path_topic', default_value='/arm2/middleware/cartesian_motion_path', description='middleware 发送连续直线笛卡尔路径的话题'),
        ...
        DeclareLaunchArgument('target_pose_executor_move_sequence_action_name', default_value='/sequence_move_group', description='MoveIt sequence action 名称'),
        DeclareLaunchArgument('target_pose_executor_sequence_pipeline_id', default_value='pilz_industrial_motion_planner', description='MoveIt sequence 使用的 planning pipeline'),
        DeclareLaunchArgument('target_pose_executor_sequence_planner_id', default_value='PTP', description='MoveIt sequence 每段使用的 planner id'),
        DeclareLaunchArgument('target_pose_executor_single_pipeline_id', default_value='pilz_industrial_motion_planner', description='MoveIt 单点规划使用的 planning pipeline'),
        DeclareLaunchArgument('target_pose_executor_single_planner_id', default_value='PTP', description='MoveIt 单点规划使用的 planner id'),
        ...
        DeclareLaunchArgument('target_pose_executor_vel_scale', default_value='0.99', description='MoveIt 速度缩放（0~1）'),
        DeclareLaunchArgument('target_pose_executor_acc_scale', default_value='0.99', description='MoveIt 加速度缩放（0~1）'),
        ...
        DeclareLaunchArgument('target_pose_executor_cartesian_max_step', default_value='0.005', description='MoveIt 笛卡尔直线插值最大步长（m）'),
        DeclareLaunchArgument('target_pose_executor_cartesian_min_fraction', default_value='0.99', description='MoveIt 笛卡尔路径最低完成比例'),
        DeclareLaunchArgument('target_pose_executor_sequence_position_tolerance', default_value='0.005', description='MoveIt sequence 途径点位置约束容差（m）'),
        DeclareLaunchArgument('target_pose_executor_sequence_orientation_tolerance', default_value='0.05', description='MoveIt sequence 途径点姿态约束容差（rad）'),
        ...
        DeclareLaunchArgument('target_pose_executor_report_enabled', default_value='true', description='是否自动记录规划轨迹的执行效果'),
        DeclareLaunchArgument('target_pose_executor_report_dir', default_value='~/.ros/rc_arm_2/trajectory_reports', description='轨迹执行报告保存目录'),
        DeclareLaunchArgument('target_pose_executor_feedback_topic', default_value='/arm_controller/execution_feedback', description='控制器期望/实际轨迹反馈话题'),
        DeclareLaunchArgument('target_pose_executor_report_label', default_value='', description='报告环境标签，例如 mujoco 或 real'),
    ...
    arm2_middleware = Node(
        ...
        parameters=[{
            ...
            'motion_path_topic': ParameterValue(LaunchConfiguration('middleware_motion_path_topic'), value_type=str),
            'cartesian_motion_target_topic': ParameterValue(LaunchConfiguration('middleware_cartesian_motion_target_topic'), value_type=str),
            'cartesian_motion_path_topic': ParameterValue(LaunchConfiguration('middleware_cartesian_motion_path_topic'), value_type=str),
    ...
    target_pose_executor = ExecuteProcess(
        cmd=[
            ...
            '--middleware-motion-path-topic',
            LaunchConfiguration('middleware_motion_path_topic'),
            '--middleware-cartesian-target-topic',
            LaunchConfiguration('middleware_cartesian_motion_target_topic'),
            '--middleware-cartesian-path-topic',
            LaunchConfiguration('middleware_cartesian_motion_path_topic'),
            ...
            '--move-sequence-action-name',
            LaunchConfiguration('target_pose_executor_move_sequence_action_name'),
            '--sequence-pipeline-id',
            LaunchConfiguration('target_pose_executor_sequence_pipeline_id'),
            '--sequence-planner-id',
            LaunchConfiguration('target_pose_executor_sequence_planner_id'),
            '--single-pipeline-id',
            LaunchConfiguration('target_pose_executor_single_pipeline_id'),
            '--single-planner-id',
            LaunchConfiguration('target_pose_executor_single_planner_id'),
            '--pos-threshold',
            ...
            '--cartesian-max-step',
            LaunchConfiguration('target_pose_executor_cartesian_max_step'),
            '--cartesian-min-fraction',
            LaunchConfiguration('target_pose_executor_cartesian_min_fraction'),
            '--sequence-position-tolerance',
            LaunchConfiguration('target_pose_executor_sequence_position_tolerance'),
            '--sequence-orientation-tolerance',
            LaunchConfiguration('target_pose_executor_sequence_orientation_tolerance'),
            '--report-enabled',
            LaunchConfiguration('target_pose_executor_report_enabled'),
            '--report-dir',
            LaunchConfiguration('target_pose_executor_report_dir'),
            '--execution-feedback-topic',
            LaunchConfiguration('target_pose_executor_feedback_topic'),
            '--report-label',
            LaunchConfiguration('target_pose_executor_report_label'),
        ],
        output='screen',
