...
def generate_launch_description():
    ...
    declared_arguments = [
        ...
        DeclareLaunchArgument("target_pose_executor_pos_threshold", default_value="0.03", description="在线追踪完成判定的位置容差（m）"),
        DeclareLaunchArgument("target_pose_executor_rot_threshold", default_value="0.3", description="在线追踪完成判定的姿态容差（rad）"),
        DeclareLaunchArgument("target_pose_executor_manual_target_pos_delta_threshold", default_value="0.0", description="手动目标更新的最小位置变化量（m）"),
        DeclareLaunchArgument("target_pose_executor_manual_target_rot_delta_threshold", default_value="0.0", description="手动目标更新的最小姿态变化量（rad）"),
        ...
        DeclareLaunchArgument("target_pose_executor_feedback_sync_mode", default_value="desync_only", description="反馈同步模式：init_only/desync_only/always"),
        DeclareLaunchArgument("target_pose_executor_feedback_position_reset_threshold", default_value="0.12", description="计划态与反馈态位置失配阈值（rad）"),
        DeclareLaunchArgument("target_pose_executor_feedback_position_reset_cycles", default_value="3", description="位置失配连续多少个周期后才重同步"),
        DeclareLaunchArgument("target_pose_executor_feedback_velocity_reset_enabled", default_value="false", description="是否启用速度失配重同步"),
        DeclareLaunchArgument("target_pose_executor_feedback_velocity_reset_threshold", default_value="1.5", description="计划态与反馈态速度失配阈值（rad/s）"),
        DeclareLaunchArgument("target_pose_executor_feedback_velocity_filter_alpha", default_value="0.2", description="反馈速度一阶低通 alpha"),
        DeclareLaunchArgument("target_pose_executor_feedback_accel_mode", default_value="zero", description="反馈加速度模式：zero/filtered"),
    ...
    target_pose_executor = ExecuteProcess(
        cmd=[
            ...
            "--manual-target-pos-delta-threshold",
            LaunchConfiguration("target_pose_executor_manual_target_pos_delta_threshold"),
            "--manual-target-rot-delta-threshold",
            LaunchConfiguration("target_pose_executor_manual_target_rot_delta_threshold"),
            "--check-period",
            ...
            "--feedback-sync-mode",
            LaunchConfiguration("target_pose_executor_feedback_sync_mode"),
            "--feedback-position-reset-threshold",
            LaunchConfiguration("target_pose_executor_feedback_position_reset_threshold"),
            "--feedback-position-reset-cycles",
            LaunchConfiguration("target_pose_executor_feedback_position_reset_cycles"),
            "--feedback-velocity-reset-enabled",
            LaunchConfiguration("target_pose_executor_feedback_velocity_reset_enabled"),
            "--feedback-velocity-reset-threshold",
            LaunchConfiguration("target_pose_executor_feedback_velocity_reset_threshold"),
            "--feedback-velocity-filter-alpha",
            LaunchConfiguration("target_pose_executor_feedback_velocity_filter_alpha"),
            "--feedback-accel-mode",
            LaunchConfiguration("target_pose_executor_feedback_accel_mode"),
        ],
        output="screen",
