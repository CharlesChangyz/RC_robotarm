...
def generate_launch_description():
    declared_arguments = [
        ...
        DeclareLaunchArgument('target_pose_executor_avoid_collisions_enabled', default_value='true', description='target_pose 执行器中 IK 是否启用避障'),
        DeclareLaunchArgument('target_pose_executor_forbidden_link_pairs', default_value='base_link:end_effector', description='机械臂/末端自碰撞禁区，格式 link_a:link_b,link_c:link_d'),
        DeclareLaunchArgument('target_pose_executor_env_forbidden_boxes_json', default_value='[]', description='环境碰撞禁区（盒体）JSON数组'),
        DeclareLaunchArgument('target_pose_executor_planning_scene_topic', default_value='/planning_scene', description='PlanningScene 更新话题'),
    ...
    target_pose_executor = ExecuteProcess(
        cmd=[
            ...
            LaunchConfiguration('target_pose_executor_forbidden_link_pairs'),
            '--env-forbidden-boxes-json',
            LaunchConfiguration('target_pose_executor_env_forbidden_boxes_json'),
